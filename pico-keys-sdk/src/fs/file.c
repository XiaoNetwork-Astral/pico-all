...
static struct {
    ...
    size_t count;
} namespaces[FILE_NAMESPACE_COUNT];
static uint16_t active_namespace;

uint16_t file_namespace_current(void) { return active_namespace; }
void file_register_namespace(uint16_t id, file_entry_t *entries, size_t count) {
    if (id >= FILE_NAMESPACE_COUNT || !entries || count < 2) return;
    namespaces[id].entries = entries;
    namespaces[id].count = count;
    for (size_t i = 0; i < count; ++i) entries[i].namespace_id = id;
}
void file_namespace_select(uint16_t id) {
    if (id >= FILE_NAMESPACE_COUNT || !namespaces[id].entries) return;
    active_namespace = id;
    file_entries = namespaces[id].entries;
    file_last = file_entries + namespaces[id].count - 1;
    MF = &file_entries[0].file;
}
static const file_entry_t *file_static_entry(const file_t *file) {
    if (!file) return NULL;
#ifndef ENABLE_EMULATION
    if (file == ef_phy) return &sef_phy;
#endif
    uintptr_t addr = (uintptr_t)file - offsetof(file_entry_t, file);
    for (size_t ns = 0; ns < FILE_NAMESPACE_COUNT; ++ns) {
        uintptr_t first = (uintptr_t)namespaces[ns].entries;
        uintptr_t end = first + namespaces[ns].count * sizeof(file_entry_t);
        if (addr >= first && addr < end && (addr - first) % sizeof(file_entry_t) == 0)
...
static bool is_parent(const file_entry_t *child, const file_t *parent) {
    ...
    return is_parent(&namespaces[child->namespace_id].entries[child->parent], parent);
}

file_t *get_parent(file_t *f) {
    const file_entry_t *entry = file_static_entry(f);
    uint8_t parent = entry ? entry->parent : 5;
    if (!f || f->namespace_id >= FILE_NAMESPACE_COUNT) return NULL;
    if (parent >= namespaces[f->namespace_id].count) parent = 0;
    return &namespaces[f->namespace_id].entries[parent].file;
}
...
#endif
    if (fid == 0x1122 && namespaces[0].entries) return &namespaces[0].entries[1].file;
    for (file_entry_t *p = file_entries; p != file_last; p++) {
...
static size_t dynamic_hash(uint16_t fid) {
    return (((uint32_t)fid | ((uint32_t)active_namespace << 16)) * 40503u) % DYNAMIC_FILE_SLOTS;
}
...
static file_t *search_dynamic_file(uint16_t fid) {
    ...
    for (size_t probes = 0; probes < DYNAMIC_FILE_SLOTS; probes++) {
        ...
        if (file->fid == fid && file->namespace_id == active_namespace) {
            return file;
        }
...
void file_for_each_dynamic(file_iter_cb cb, void *ctx) {
    for (size_t i = 0; i < DYNAMIC_FILE_SLOTS; i++) {
        if (dynamic_file[i].namespace_id == active_namespace &&
            dynamic_file[i].fid != DYNAMIC_FID_EMPTY &&
...
static void clear_index(void) {
    for (size_t ns = 0; ns < FILE_NAMESPACE_COUNT; ++ns)
        for (size_t i = 0; i < namespaces[ns].count; ++i)
            if (namespaces[ns].entries[i].type & FILE_DATA_FLASH)
                namespaces[ns].entries[i].data = NULL;
    memset(dynamic_file, 0, sizeof(dynamic_file));
    dynamic_files = 0;
}
void file_initialize_flash(bool hard) {
    if (!hard) return; // Re-scanning is performed by file_scan_flash; never discard another app's index.
    for (file_entry_t *f = file_entries; f != file_last; ++f)
        if ((f->type & FILE_DATA_FLASH) && !(f->type & FILE_PERSISTENT))
            flash_clear_file(&f->file);
    for (size_t i = 0; i < DYNAMIC_FILE_SLOTS; ++i) {
        file_t *f = &dynamic_file[i];
        if (f->namespace_id == active_namespace && f->fid != DYNAMIC_FID_EMPTY && f->fid != DYNAMIC_FID_DELETED) {
            flash_clear_file(f);
            f->fid = DYNAMIC_FID_DELETED;
            if (dynamic_files) --dynamic_files;
        }
    }
...
static void scan_region_internal(bool persistent, bool allow_force_restore) {
    ...
    for (uintptr_t base = flash_read_uintptr(endp); base != 0; base = flash_read_uintptr(base)) {
        size_t minimum_header = 2 * sizeof(uintptr_t) + FILE_RECORD_ID_SIZE + sizeof(uint16_t);
        ...
        uint32_t identity = flash_read_uint32(base + 2 * sizeof(uintptr_t));
        uint16_t fid = (uint16_t)identity;
        uint16_t ns = (uint16_t)(identity >> 16);
        ...
        size_t record_header = 2 * sizeof(uintptr_t) + FILE_RECORD_ID_SIZE + length_size;
        if (ns >= FILE_NAMESPACE_COUNT || !namespaces[ns].entries || fid == DYNAMIC_FID_EMPTY || fid == DYNAMIC_FID_DELETED || record_header > endp - base || length > endp - base - record_header || (next_addr != 0 && (next_addr < startp || next_addr >= base)) || (prev_addr != endp && (prev_addr <= base || prev_addr > endp))) {
            printf("ERROR: invalid flash record at %x\n", (unsigned int)base);
            corrupted = true;
            break;
        }
        printf("[%x] scan fid %x, len %lu\n", (unsigned int)base, fid, (unsigned long)length);
        uint16_t previous_namespace = active_namespace;
        ...
        if (file) {
            file->data = (uint8_t *) (base + 2 * sizeof(uintptr_t) + FILE_RECORD_ID_SIZE);
        }
        file_namespace_select(previous_namespace);
        if (!persistent) {
            num_files++;
        }
...
void file_scan_flash(void) {
    ...
    uint32_t format = flash_read_uint32(end_rom_pool + sizeof(uintptr_t));
    if (format != 0x50414c31u) {
        if (flash_read_uintptr(end_rom_pool) != 0 || flash_read_uintptr(end_data_pool) != 0) {
            printf("Pico All requires fresh storage; existing data was not modified\n");
            while (1) { /* Stop before touching an incompatible filesystem. */ }
        }
        flash_program_word(end_rom_pool + sizeof(uintptr_t), 0x50414c31u);
        flash_commit();
    }
    printf("SCAN\n");
    scan_region(true);
...
file_t *file_new(uint16_t fid) {
    ...
    f->data = NULL;
    f->fid = fid;
    f->namespace_id = active_namespace;
    dynamic_files++;
    return f;
}
...
int flash_clear_file(file_t *file) {
    ...
    uintptr_t base_addr = (uintptr_t)(file->data - 2 * sizeof(uintptr_t) - FILE_RECORD_ID_SIZE);
