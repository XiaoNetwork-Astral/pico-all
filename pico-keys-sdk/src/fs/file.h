...
#define FILE_NAMESPACE_COUNT 4u
#define FILE_RECORD_ID_SIZE sizeof(uint32_t)
extern void file_register_namespace(uint16_t id, file_entry_t *entries, size_t count);
extern uint16_t file_namespace_current(void);
extern void file_namespace_select(uint16_t id);
extern void file_namespaces_init(void);
