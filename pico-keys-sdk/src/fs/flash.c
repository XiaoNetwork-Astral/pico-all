...
static size_t flash_record_size(uintptr_t base) {
    return 2 * sizeof(uintptr_t) + FILE_RECORD_ID_SIZE + flash_record_length_size(base) + flash_record_payload_size(base);
}
...
static uintptr_t allocate_free_addr(uint32_t size, bool persistent, uintptr_t *publish_at) {
    ...
    size_t overhead = 2 * sizeof(uintptr_t) + FILE_RECORD_ID_SIZE + length_size;
    size_t real_size = 0;
...
static int flash_write_data_to_file_internal(file_t *file, const_byte_array_t data, uint32_t offset, bool partial) {
    ...
    file_t new_file = {
        .data = (uint8_t *)new_addr + 2 * sizeof(uintptr_t) + FILE_RECORD_ID_SIZE,
        .fid = file->fid,
    ...
    int r = flash_program_word(new_addr + 2 * sizeof(uintptr_t), ((uint32_t)new_file.namespace_id << 16) | new_file.fid);
    ...
    if (replacing && 2 * sizeof(uintptr_t) + FILE_RECORD_ID_SIZE + length_size + new_size > FLASH_SECTOR_SIZE && !flash_commit_sync(5000u)) {
        return PICOKEYS_ERR_MEMORY_FATAL;
    }
    *file = new_file;
    return PICOKEYS_OK;
