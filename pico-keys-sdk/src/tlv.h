...
typedef byte_array_t tlv_ctx_t;
...
extern int tlv_ctx_init(byte_array_t data, tlv_ctx_t *);
extern int tlv_ctx_clear(tlv_ctx_t *ctx);
extern size_t tlv_len(const tlv_ctx_t *ctx);
extern uint32_t tlv_get_uint(tlv_ctx_t *ctx);

extern int tlv_walk(const tlv_ctx_t *ctxi, uint8_t **p, tlv_item_t *item);
