...
int hsm_cmd_reset_retry(void) {
    ...
    if (P1(apdu) == 0x0 || P1(apdu) == 0x2) {
        ...
        if (P1(apdu) == 0x0) {
            ...
            uint16_t r = hsm_check_pin(file_sopin, CONST_BYTE_ARRAY(apdu.data, so_pin_len));
            if (r != 0x9000) {
                return r;
            }
        ...
        if (hsm_pin_reset_retries(file_pin1, true) != PICOKEYS_OK) {
        ...
        pin_derive_session(CONST_BYTE_ARRAY(apdu.data + (apdu.nc - newpin_len), newpin_len), hsm_session_pin);
