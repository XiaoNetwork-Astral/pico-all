...
int hsm_cmd_change_pin(void) {
    if (P1(apdu) == 0x0) {
        if (P2(apdu) == 0x81 || P2(apdu) == 0x88) {
            ...
            int r = hsm_check_pin(file_pin, CONST_BYTE_ARRAY(apdu.data, pin_len));
            if (r != 0x9000) {
                return r;
            }
            ...
            if (P2(apdu) == 0x81) {
                pin_derive_session(CONST_BYTE_ARRAY(apdu.data + pin_len, new_pin_len), hsm_session_pin);
