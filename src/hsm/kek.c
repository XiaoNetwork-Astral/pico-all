...
mse_t hsm_mse = { .init = false };

int hsm_mse_decrypt_ct(byte_array_t data) {
    if (!data.data || data.len < 16) {
        return PICOKEYS_WRONG_DATA;
    }
    mbedtls_chachapoly_context chatx;
    mbedtls_chachapoly_init(&chatx);
    mbedtls_chachapoly_setkey(&chatx, hsm_mse.key_enc + 12);
    int ret = mbedtls_chachapoly_auth_decrypt(&chatx, data.len - 16, hsm_mse.key_enc, hsm_mse.Qpt, 65, data.data + data.len - 16, data.data, data.data);
...
int store_mkek(const uint8_t *mkek) {
    ...
    if (has_session_pin) {
        ...
        encrypt_with_aad(hsm_session_pin, CONST_BYTE_ARRAY(mkek, MKEK_SIZE), 2, tmp_mkek + 1);
