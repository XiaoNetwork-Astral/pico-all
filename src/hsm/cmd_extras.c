...
int cmd_extras(void) {
    ...
    if (hsm_wait_button_pressed() == true) {
    ...
    if (cmd == CMD_DYNOPS) {   //dynamic options
        ...
        if (P2(apdu) == SECURE_LOCK_KEY_AGREEMENT) { // Key Agreement
            ...
            memcpy(hsm_mse.Qpt, apdu.data, sizeof(hsm_mse.Qpt));
            ...
            ret = mbedtls_hkdf(mbedtls_md_info_from_type(MBEDTLS_MD_SHA256), NULL, 0, buf, olen, hsm_mse.Qpt, sizeof(hsm_mse.Qpt), hsm_mse.key_enc, sizeof(hsm_mse.key_enc));
            mbedtls_platform_zeroize(buf, sizeof(buf));
            if (ret != 0) {
                mbedtls_ecdh_free(&hkey);
            ...
            uint16_t opts = get_device_options();
            int ret = hsm_mse_decrypt_ct(BYTE_ARRAY(apdu.data, apdu.nc));
            if (ret != 0) {
                return SW_WRONG_DATA();
            }
