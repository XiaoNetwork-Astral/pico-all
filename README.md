...
- OpenPGP：P-256 签名与 ECDH、RSA-2048 认证签名，以及原签名密钥保留
- PIV：P-256 签名与 ECDH、RSA-2048 签名/私钥解密、生成证书读取，以及 OpenPGP 数据保留
- HSM：P-256 签名与 ECDH、AES-256 CBC 往返、CMAC 重复性与消息差异、按键确认与超时拒绝
- 管理接口：按键确认后允许原样配置写入，不确认时超时拒绝；重启和更新固件后的数据保存已验证

U2F、断电恢复和更完整的单应用重置场景仍在补充测试；RSA-2048 卡内生成可能耗时数分钟，一次 OpenPGP 实测约 162 秒
...
普通请求保持绿色呼吸且不重置周期；按键提示与更新模式优先于完成提示；蓝灯在固件请求进入 BOOTSEL 前设置，进入 ROM 后由启动程序接管，断电后灯光不保留
